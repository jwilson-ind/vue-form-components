<template>
    <div :class="{error: inputErrors?.length}">
        ...
        <div v-else>
            ...
            <Select v-if="select" :id="id" :type="select.type" :multiple="select.multiple" :url="select.url" :field="select.field" :label="select.label" :options="select.options" :filters="select.filters" :search-columns="select.searchColumns" :limit="select.limit" :preload="select.preload" v-model="thisModelValue" :disabled="disabled" @update:modelValue="$emit('update:modelValue', $event)" :placeholder="placeholder ? placeholder : label"></Select>
            <div v-else-if="type === 'attachments'">
                ...
                <FileUpload multiple :id="id" v-model="thisModelValue" :acceptable-types="acceptableTypes" :max-size="maxSize" :placeholder="placeholder" :disabled="disabled" @update:modelValue="$emit('update:modelValue', $event)" />
            </div>
            <Wysiwyg v-else-if="type === 'wysiwyg'" v-model="thisModelValue" @update:modelValue="$emit('update:modelValue', $event)" />
            <FileUpload v-else-if="type === 'image' || type === 'file'" :id="id" v-model="thisModelValue" :acceptable-types="acceptableTypes" :max-size="maxSize" :placeholder="placeholder" :disabled="disabled" @update:modelValue="$emit('update:modelValue', $event)" />
            ...
            <input v-else :id="id" class="input-text" :type="parsedType" :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" :placeholder="placeholder ? placeholder : label" :autocomplete="autocomplete" :required="required" :disabled="disabled" :min="min" :max="max" :step="step"/>
        </div>
...
const emit = defineEmits(['update:modelValue', 'remove-attachment']);
...
const minMaxStepAttributes = computed(() => {
    ...
    return Object.fromEntries(
        props.type.split(':').map(attribute => { // ['min=1', 'max=1, 'step=1']
            const nameValue = attribute.split('='); // 'min=1'
            return [nameValue[0], nameValue[1] ?? null]; // ['min', '1']
        })
    );
})
